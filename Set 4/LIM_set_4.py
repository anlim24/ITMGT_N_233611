...
def bin_to_dec(binary_string):
    ...
    for i in range(len(binary_string)):
        base_ten_integer += int(binary_string[i])*(2**(len(binary_string)-1-i))

    return base_ten_integer
...
def dec_to_bin(number):
    ...
    while max_power >= 0:
        binary_digit = number_copy // (2**max_power)
        binary_string += str(binary_digit)
        number_copy = number_copy - binary_digit*(2**max_power)
        max_power -= 1
...
def telephone_cipher(message):
    ...
    for char in message:
        converted_char = encoder_dict.get(char)
        converted_message += str(converted_char)
        
        if index+1 < len(message):
            if (encoder_dict.get(message[index]))[0] in encoder_dict.get(message[index+1]):
                converted_message += "_"
...
def telephone_decipher(telephone_string):
    ...
    while index < len(telephone_string):
        ...
        while index < len(telephone_string)-1 and telephone_string[index] == telephone_string[index+1]:
            index += 1

        list.append(telephone_string[start_index:index + 1])

        char_numerical_string = ''.join(list)
        decipher_message += decipher_dict[char_numerical_string]
