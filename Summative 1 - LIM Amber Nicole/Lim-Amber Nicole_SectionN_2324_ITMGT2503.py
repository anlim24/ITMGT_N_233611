...
with open(r"C:\Users\Amber\Downloads\script.json", "r") as file:
...
## convert to string -- convert value to string to .upper then remove stuff
#### Convert all characters to uppercase characters
#### Remove the following pieces of text: <P>, </P>, <B>, </B>, <I>, </I>
#### Replace all the unnecessary punctuation symbols denoted in the string below -- convert to space
...
#Uppercase
jsondata = jsondata.upper()

#Remove


#Replace
jsondata = jsondata.replace(unnecessary_punctuation, "")
...
list_of_possible_R = [ ]
list_of_possible_C = [ ]
for F in range(1500//50): #max fighters possible
    for R in range(1500//45):
        for C in range(12000//300):
            if 1000*F + 600*R + 750*C <= 18000:
                if 500*F + 750*R + 300*C <= 12000:
                    if 50*F + 45*R + 75*C <= 1500:
                        list_of_possible_Z.append(10*F + 12*R + 16*C)
                        list_of_possible_F.append(F)
                        list_of_possible_R.append(R)
                        list_of_possible_C.append(C)

optimal_Z = max(list_of_possible_Z)

index = list_of_possible_Z.index(max(list_of_possible_Z))

optimal_F = list_of_possible_F[index]
optimal_R = list_of_possible_R[index]
optimal_C = list_of_possible_C[index]
...
def Collatz(start_number):
    ...
    sequence_sum = 0
    for i in range(collatz_length):
        sequence_sum = sequence_sum + collatz_sequence[i]
        
    dictionary = {
        "collatz_sequence": collatz_sequence,
        "collatz_length": collatz_length, 
        "max_collatz": max_collatz,
        "seuqence_sum": sequence_sum
    }
    return dictionary
...
def Collatz_winner(number_list):
    ...
    for start_number in number_list:
        ...
        collatz_length = len(collatz_sequence)
        list_collatz_length.append(collatz_length)
    
        max_collatz = max(collatz_sequence)
        list_max_collatz.append(max_collatz)

#===================================================================================
    for x in list_collatz_length:
        for y in list_max_collatz:
            if x == max(list_collatz_length) and y == max(list_max_collatz):
                if list_collatz_length.index(x) == list_max_collatz.index(y):
                    index = list_max_collatz.index(y)
                    winner = number_list[index]
                    return winner

                else:
                    return None

#how do i do it if may similar na same index for length/max rawr
